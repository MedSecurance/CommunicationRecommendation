...
from backend.Communication_features import Communication_wifi, Communication_zigbee
from backend.Connectivity_features import Connectivity_features_basic, Connectivity_features_time, \
    Connectivity_features_flags_bytes
from backend.Dynamic_features import Dynamic_features
from backend.Layered_features import L3, L4, L2, L1
from backend.Supporting_functions import get_protocol_name, get_flow_info, get_flag_values, compare_flow_flags, \
...
class Feature_extraction():
    ...
    def pcap_evaluation(self,pcap_file,csv_file_name):
        ...
        columns = ["src_mac","dst_mac",
            "ts","flow_duration","Header_Length",
        ...
        while last_row<len(processed_df):
            ...
            # print(f'{sliced_df = }')
            # Costas fix issue when estimating mean on a string column
            tmp_sliced_df = sliced_df.drop(["Protocol_name", "src_mac", "dst_mac"], axis=1)
            # tmp_sliced_df = sliced_df.columns[~sliced_df.columns.isin(['Protocol_name', 'src_mac', 'dst_mac'])]
            sliced_df = pd.DataFrame(tmp_sliced_df.mean()).T# mean 
            # sliced_df = pd.DataFrame(sliced_df.filter(regex="Protocol_name|src_mac|dst_mac").mean()).T
            sliced_df["Protocol_name"] = processed_df["Protocol_name"]
            sliced_df["src_mac"] = processed_df["src_mac"]
            sliced_df["dst_mac"] = processed_df["dst_mac"]              
