...
def get_color_for_type(type_name):
    ...
    # Hardcoded Advertising for some types
    if type_name == '(others)':
        return '#bdbdbd'
...
def show_data_usage_table(mac_addr, last_n_seconds=20, group_by_col='reg_domain', show_empty=True, graph_type="Domains"):
    ...
    st.download_button(
        "Export data to csv",
        csv,
        f"{graph_type}_{mac_addr}_report_{date_time}.csv",
        "text/csv",
        key='download-overview'
    )
    
    row_widths = [0.4, 0.3, 0.3]
...
if option != default_option:
    ...
    st.query_params['mac_addr'] = mac_addr
