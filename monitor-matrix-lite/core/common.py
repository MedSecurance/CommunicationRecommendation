...
def http_request_medsec(method='get', field_to_extract='', args=[], kwargs={}) -> str:
    """
    Issues an HTTP request and parse the returned contents.
    ...
    # Check if success
    if 'devices' in response_dict and len(response_dict['devices']) < 1:
        err_msg = ''
        if 'error' in response_dict:
            err_msg = response_dict['error']
        # Ignore the most common error (which is not an error because it takes
    ...
    # Return the field
    if field_to_extract:
        device = response_dict['devices'][0]
        if field_to_extract not in device:
            log(f'[http_request] Error: request with args {args} did not return the field {field_to_extract}')
            raise IOError
        return f"{device[field_to_extract]}"

    return ''


def http_email_medsec(method='post', args=[], kwargs={}) -> str:
    """
