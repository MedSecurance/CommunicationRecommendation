...
def show():
    ...
    with st.sidebar:
        ...
        with global_state.global_state_lock:
            ...
            st.session_state['should_refresh'] = global_state.is_refresing
            st.session_state['user_ip_range'] = get_user_ip_range()
            global_state.user_ip_range = st.session_state['user_ip_range']
        ...
        auto_refresh = st.session_state['should_refresh']
        ...
        if auto_refresh:
            with global_state.global_state_lock:
                ...
                global_state.is_refresing = st.session_state['should_refresh']
        else:
            with global_state.global_state_lock:
                st.session_state['should_refresh'] = False
                global_state.is_refresing = st.session_state['should_refresh']
        
        # Allow the user to define the IP range to monitor
        st.markdown('<div class="tight-text">Select IP to Monitor:</div>', unsafe_allow_html=True)
        stored_user_ips = get_user_ip_range()
        ip_list = create_ip_range()

        df = pd.DataFrame({
            "IP": ip_list,
            "Selected": [ip in get_user_ip_range() for ip in ip_list]
        })
        ...
        edited_df = st.data_editor(
            df,
            column_config={
                "IP Address": st.column_config.TextColumn(width="50px"),
                "Selected": st.column_config.CheckboxColumn(width="fit_content"),
            },
            use_container_width=True,
            hide_index=True
        )

        # Extract selected IPs
        selected_ips = edited_df[edited_df["Selected"]]["IP"].tolist()
        # print(f"{selected_ips = }")
        
        if selected_ips:
            # print(f"selected_ips: {selected_ips} - {stored_user_ips}")
            with global_state.global_state_lock:
                st.session_state['user_ip_range'] = selected_ips
                global_state.user_ip_range = st.session_state['user_ip_range']
            with model.db:
                for ip in selected_ips:
                    if ip not in stored_user_ips:
                        model.UserRange.create(ip_addr = ip)      
        else:
            # print(f"selected_ips else: {selected_ips} - {stored_user_ips}")
            with global_state.global_state_lock:
                st.session_state['user_ip_range'] = []
                global_state.user_ip_range = st.session_state['user_ip_range']
            with model.db:
                for ip in stored_user_ips:
                    model.Device.delete().where(model.Device.ip_addr == ip).execute()
                    model.UserRange.delete().where(model.UserRange.ip_addr == ip).execute()
        with model.db:
            model.Device.delete().where(model.Device.ip_addr == "").execute()
            for ip in stored_user_ips:
        ...
        st.markdown('## 📊 Overall Statistics\n' +
                   f' * Inspected devices: {get_inspected_device_count()-1}\n' +
...
def get_user_ip_range():
    with model.db:
        t = model.UserRange.select(model.UserRange.ip_addr)
        user_ip_range = [x.ip_addr for x in t]
        # user_ip_range.add(global_state.gateway_ip_addr)
    return user_ip_range


@st.cache_data
def create_ip_range():
    ip = ".".join(global_state.host_ip_addr.split(".")[:-1])
    ip_list = []
    for i in range(1, 255):
        ip_list.append(f"{ip}.{i}")
    return ip_list
