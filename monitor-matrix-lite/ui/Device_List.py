...
def show_device(device: model.Device):
    ...
    if device.is_inspected:
        ...
        tt = f'<a style="color: blue; font-size: x-large;" href="{global_state.BASE_PATH}/Device_Details?{params}" target="_blank">{product_name}</a>'
        st.html(tt)
    else:
        st.markdown(f'#### {product_name}')

    # Show possible device identity if the data is available and the user has not set the product name yet
    if device.friendly_product and device.product_name == '' and skip_possible:
        st.caption(
...
with model.db:
    ...
    exported_devices = model.Device.select(model.Device.mac_addr, model.Device.ip_addr, model.Device.product_name, model.Device.friendly_product)

df = pd.DataFrame(list(exported_devices.dicts()))
df_csv = df.to_csv(index=False, sep="\t").encode('utf-8')
...
st.download_button(
    "Export data to csv",
    df_csv,
    f"Device_list_{date_time}.csv",
    "text/csv",
    key='download-devices'
)
st.divider()
